...
(function ($) {
    ...
    $.widget('blueimpUI.fileupload', $.blueimp.fileupload, {
                ...
                if (data.context) {
                    data.context.each(function (index) {
                        ...
                        if (file.error) {
                        	...
                        	//Added by Ed Brown for vBulletin (modified by Glenn)
				var editorId = $('#upload-dialog').data('editorId');
				var formId = $('#' + editorId).closest('form').attr('id');
                        	$('<input type="hidden" name="filedataids[]" />')
                        		.val(data.result[index].filedataid)
                        		.data('url', data.result[index].url)
                        		.data('thumbnail_url', data.result[index].thumbnail_url)
                        		.data('name', data.result[index].name)
                        		.appendTo('#' + formId);
                        	$('<input type="hidden" name="filename_' + data.result[index].filedataid + '" />')
                        		.val(data.result[index].name)
                        		.data('name', data.result[index].name)
                        		.appendTo('#' + formId);
                        }
        ...
        _startHandler: function (e) {
            e.preventDefault();
            var that = $(this),
				tmpl = that.closest('.template-upload'),
                data = tmpl.data('data');
			...
			if (typeof FileReader !== 'undefined' && typeof FileReader === 'function')
			{
				...
				var validation = (function(){
					var validate = function (imageWidth, imageHeight, imageSize){
						var maxWidth = vBulletin.permissions[fileExt]['values']['max_width'];
						var maxHeight = vBulletin.permissions[fileExt]['values']['max_height'];
						var maxSize = vBulletin.permissions[fileExt]['values']['max_size'];
						var error = vBulletin.phrase.get('error_uploading_image');
						if ((maxWidth > 0 && imageWidth > maxWidth) || (maxHeight > 0 && imageHeight > maxHeight))
						{
							error = vBulletin.phrase.get('upload_exceeds_dimensions', maxWidth,
									maxHeight, imageWidth, imageHeight);
							openAlertDialog({
								'title': vBulletin.phrase.get('upload'),
								'message': vBulletin.phrase.get(error),
								'iconType': 'warning'
							});
							return false;
						}
						else if (maxSize > 0 && imageSize > maxSize)
						{
							error = vBulletin.phrase.get('upload_file_exceeds_limit', imageSize, maxSize);
							openAlertDialog({
								'title': vBulletin.phrase.get('upload'),
								'message': vBulletin.phrase.get(error),
								'iconType': 'warning'
					...
					return {
						submit: function(width, height, size){
							if(validate(width, height, size) && data && data.submit&&!data.jqXHR)
							{
								data.jqXHR=data.submit();
								that.fadeOut()
							}
				...
				reader.onloadend = function() {
					...
					image.onload = function() {
						...
						if (typeof vBulletin.permissions[fileExt] === 'undefined' ||
							typeof vBulletin.permissions[fileExt]['set'] === 'undefined' ||
							!vBulletin.permissions[fileExt]['set'])
						{
							vBulletin.permissions[fileExt] = {};
							vBulletin.permissions[fileExt]['ext'] = fileExt;
							vBulletin.permissions[fileExt]['values'] = {};
							vBulletin.permissions[fileExt]['set'] = false;
							var data = {data:{
								extension: fileExt,
								uploadFrom: $('input[type=hidden][name=uploadFrom]').val()
							}};
							$.ajax({
								url: vBulletin.getAjaxBaseurl() + '/ajax/api/content_attach/getAttachmentPermissions',
								data: data,
								type: 'POST',
								dataType:'json',
								async: false,
								success: function(response){
									if (!response.errors)
									{
										for(var key in response)
										{
											...
											{
												vBulletin.permissions[fileExt]['values'][key] = response[key];
											}

										}
										vBulletin.permissions[fileExt]['set'] = true;
										validation.submit(imageWidth, imageHeight, imageSize);
									}
									else
									{
										var error = response.errors[0][0];
										openAlertDialog({
											'title': vBulletin.phrase.get('upload'),
											'message': vBulletin.phrase.get(error),
											'iconType': 'warning'
