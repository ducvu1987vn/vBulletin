	...
	CKEDITOR.bbcodeDataProcessor.prototype = {
		doServerParse: function(message, wysiwyg)
		{
			...
			{/*
				...
				var response = '';
				if (responseXML && responseXML.getElementsByTagName && responseXML.getElementsByTagName('message')[0] && responseXML.getElementsByTagName('message')[0].firstChild)
				{
					response = unescapeResponse(responseXML.getElementsByTagName('message')[0].firstChild.nodeValue);
				}
			}
			else
			{
