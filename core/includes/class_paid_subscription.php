...
class vB_PaidSubscription
{
	...
	*/
	function build_user_subscription($subscriptionid, $subid, $userid, $regdate = 0, $expirydate = 0, $checkperms = true)
	{
		...
		$currentsubscription = vB::getDbAssertor()->getRow('subscriptionlog', array('userid' => $userid, 'subscriptionid' => $subscriptionid));
		...
		if ($user['userid'] AND $sub['subscriptionid'])
		{
			...
			if ($subid != -1 AND is_array($tmp["$subid"]))
			{
				...
				{	// new expiration is still after today so just decremement and return
					vB::getDbAssertor()->update('subscriptionlog', array('expirydate' => $new_expires), array('subscriptionid' => $subscriptionid, 'userid' => $userid));
					return;
				}
			}
			unset($tmp);
			...
			$userdm->save();
			unset($userdm);
			vB::getDbAssertor()->update('subscriptionlog', array('status' => 0), array('subscriptionid' => $subscriptionid, 'userid' => $userid));
			...
			$mysubs = vB::getDbAssertor()->assertQuery('subscriptionlog', array('status' => 1), array('userid' => $userid));
