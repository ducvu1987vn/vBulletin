...
function construct_page_nav($pagenumber, $perpage, $results, $address, $address2 = '', $anchor = '', $seolink = '', $objectinfo = array(), $pageinfo = array())
{
	...
	foreach ($pages AS $foo => $curpage)
	{
		...
		// Legacy Hook 'pagenav_page' Removed //
		...
		if ($pagenumber != $totalpages)
		{
			...
			if ($seolink)
			{
				...
				$lastaddress = vB5_Route::buildUrl($seolink, $objectinfo, $pageinfo);
			}
			$show['last'] = true;
		}

		if (abs($curpage - $pagenumber) >= $options['pagenavpages'] AND $options['pagenavpages'] != 0)
		{
			// generate relative links (eg. +10,etc).
			if (in_array(abs($curpage - $pagenumber), $options['pagenavsarr']) AND $curpage != 1 AND $curpage != $totalpages)
			{
	...
	// Legacy Hook 'pagenav_complete' Removed //
