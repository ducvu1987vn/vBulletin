...
// start the navbar
$navbits = array('usercp.php' . vB::getCurrentSession()->get('sessionurl_q') => $vbphrase['user_control_panel']);
...
// Legacy Hook 'paidsub_start' Removed //
$lengths = array(
	'D' => $vbphrase['day'],
	'W' => $vbphrase['week'],
	'M' => $vbphrase['month'],
...
if ($_REQUEST['do'] == 'list')
{
	...
	// Legacy Hook 'paidsub_list_start' Removed //
	...
	foreach ($subobj->subscriptioncache AS $subscription)
	{
		...
		if (isset($subscribed["$subscription[subscriptionid]"]))
		{
			...
			// Legacy Hook 'paidsub_list_activebit' Removed //
		...
		if ($subscription['active'])
		{
			...
			// Legacy Hook 'paidsub_list_availablebit' Removed //
...
if ($_POST['do'] == 'order')
{
	$vbulletin->input->clean_array_gpc('p', array(
		'subscriptionids'	=> vB_Cleaner::TYPE_ARRAY_NOHTML,
		'currency'			=> vB_Cleaner::TYPE_ARRAY_NOHTML,
	));

	if (empty($vbulletin->GPC['subscriptionids']))
	{
		eval(standard_error(fetch_error('invalidid', $vbphrase['subscription'])));
	}
	else
	{
	...
	// Legacy Hook 'paidsub_order_start' Removed //
	...
	while ($method = $db->fetch_array($methods))
	{
		...
		if ($costs["$subscriptionsubid"]['cost']["$currency"] > 0)
		{
			...
			if (!empty($form))
			{
				...
				// Legacy Hook 'paidsub_order_paymentbit' Removed //
	...
	$navbits['payments.php' . vB::getCurrentSession()->get('sessionurl_q')] = $vbphrase['paid_subscriptions'];
...
if (!empty($page_templater))
{
	...
	// Legacy Hook 'paidsub_complete' Removed //
